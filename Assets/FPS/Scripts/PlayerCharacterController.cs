...
public class PlayerCharacterController : MonoBehaviour
{
    ...
    [SerializeField] private Quaternion offSet = new Quaternion(0.707f, -0.707f, 0, 0);
    ...
    void HandleCharacterMovement()
    {
        ...
        */
        ...
        {
            // rotate the transform with the input speed around its local Y axis
            transform.localRotation(new Vector3(0, GyroManager.Instance.GetGyroRotation().eulerAngles.y * rotationSpeed * RotationMultiplier, 0));
        }

        // vertical camera rotation
        {
            // add vertical inputs to the camera's vertical angle
            m_CameraVerticalAngle += GyroManager.Instance.GetGyroRotation().eulerAngles.x * rotationSpeed * RotationMultiplier;

            // limit the camera's vertical angle to min/max
            m_CameraVerticalAngle = Mathf.Clamp(GyroManager.Instance.GetGyroRotation().eulerAngles.x, -89f, 89f);

            // apply the vertical angle as a local rotation to the camera transform along its right axis (makes it pivot up and down)
            //playerCamera.transform.localEulerAngles = new Vector3(m_CameraVerticalAngle, 0, 0);
        }
        */
        Quaternion quaternion = GyroManager.Instance.GetGyroRotation() * baseRotation;
        //transform.localRotation = Quaternion.Euler(0, quaternion.eulerAngles.y,0);
        //transform.localEulerAngles = new Vector3(quaternion.eulerAngles.x, Mathf.Clamp(quaternion.eulerAngles.y, -89f, 89f), quaternion.eulerAngles.z);
        
        
        transform.localRotation = quaternion;
        //float clampedAngle = Mathf.Clamp(transform.localRotation.eulerAngles.y, -89f, 89f);
        //transform.localEulerAngles = new Vector3(transform.localEulerAngles.x, clampedAngle, transform.localEulerAngles.z);
        //playerCamera.transform.localRotation = quaternion;

        //playerCamera.transform.localEulerAngles = new Vector3(quaternion.eulerAngles.x, quaternion.eulerAngles.y, quaternion.eulerAngles.z);
        //playerCamera.transform.localEulerAngles = new Vector3(GyroManager.Instance.GetGyroRotation().eulerAngles.x, -GyroManager.Instance.GetGyroRotation().eulerAngles.y, -GyroManager.Instance.GetGyroRotation().eulerAngles.z);
